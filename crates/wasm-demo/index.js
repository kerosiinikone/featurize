...
/**
 * Returns raw 300x300 RGBA canvas data for Rust pipeline processing.
 * Also displays a scaled preview on the scaled canvas.
 * @param {object} mainContext - The 2d context of the source canvas.
 * @param {object} cropContext - The 2d context of an intermediate hidden canvas (unused now).
...
export function cropScaleGetImageData(mainContext, cropContext, scaledContext) {
  const rawData = new Float32Array(mainContext.getImageData(0, 0, 300, 300).data);

  const cropEl = cropContext.canvas;
  cropContext.fillStyle = "rgba(255, 255, 255, 255)";
  ...
  scaledContext.fillStyle = "rgba(255, 255, 255, 255)";
